...
이용 대시보드와 최근 7일·30일·전체 기간 필터를 제공합니다. 문장 만들기 시 MongoDB에 기록을 저장합니다.
같은 보드의 재시도는 중복 집계하지 않으며 보드를 변경하면 새 기록이 됩니다.

MongoDB 사용자 로그인과 사용자별 기록 분리를 지원합니다. 역할별 화면 권한,
회원가입, 기간 필터, 카드 업로드, TTS 성공 기록은 아직 구현하지 않았습니다.
한국어 음성 재생은 브라우저·운영체제의 한국어 TTS 지원이 필요합니다.
Qwen 모델 기능은 현재 꺼져 있으며 외부 AI API를 호출하지 않습니다.
외부 API 키나 유료 가입은 필요하지 않습니다.

기본 데모 로그인은 아이디 `demo`, 비밀번호 `demo1234`입니다. 운영 환경에서는
