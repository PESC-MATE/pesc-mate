...
문서 내용과 인증 정보를 제외한 `DB CRUD | 작업 | 컬렉션` 형식의 변경 로그만 출력합니다.
반복되는 `READ` 로그는 디버그 레벨에서만 확인할 수 있습니다.
...
새로고침됩니다.
백엔드는 `backend/app`만 감시하므로 로컬 `.venv`나 테스트 캐시를
추적하지 않으며, Docker Desktop에서 메모리 부족으로 감시기가 종료되는
문제를 방지하기 위해 폴링 방식을 사용합니다.
